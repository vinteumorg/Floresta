...
lint:
    ...
    # Run linting on `floresta-chain` with the `flat-chainstore` feature.
    cargo +nightly clippy -p floresta-chain --all-targets \
        --features bitcoinconsensus,metrics,test-utils,flat-chainstore

    # Run linting on `florestad` with the `flat-chainstore` feature.
    ...
    # Run linting in floresta-node` with the `flat-chainstore` feature.
