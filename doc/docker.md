...
Here's a breakdown of the configuration:
 - For the command, there are a couple of options that are worth noticing:
   - `--data-dir /data/.floresta` specifies florestad's data directory. Here, florestad will store its blockchain data, wallet files, and other necessary data.
     You should mount a volume to this path to persist the data outside the container.
   - `-c /data/config.toml` specifies the path to the configuration file inside the container. You should mount your configuration file to this path using a volume.
   By default, Floresta looks for a configuration file at the datadir if no configuration file is specified.
   - `-n <network>` specifies the Bitcoin network to connect to (mainnet, testnet, regtest). Make sure this matches your configuration file.
...
To use the RPC, you can use a command like this:
...
Floresta also (optionally) provide [Prometheus](https://prometheus.io/) metrics endpoint, which you can enable at compile time. If you want a quick setup with Grafana, we provide a [docker-compose.yml](docker-compose.yml) for that as well. Just use:
