...
Sends a request to the node for rescan the blockchain searching for transactions related to the wallets cached addresses.

## Arguments

* `start_height` (numeric, optional, default=`0`): The initial block height to start the blockchain rescan.

* `stop_height` (numeric, optional, default=`0`): The height limit to stop the rescanning.
...
Under the hood this uses an [Exponential distribution](https://en.wikipedia.org/wiki/Exponential_distribution) [cumulative distribution function (CDF)](https://en.wikipedia.org/wiki/Cumulative_distribution_function) where the parameter $\lambda$ (rate) is $\frac{1}{600}$ (1 block every 600 seconds, 10 minutes).
  ...
  - `none`: Doesnt apply any lookback seconds.
...
If `timestamp` is true, apart the previous rules, if any of the values is smaller than that of the genesis block (1231006505) or greater than the tip time, the execution will also fail.
