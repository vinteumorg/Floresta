...
Sends a request to the node for rescan the blockchain searching for transactions realted to the wallets cached addresses.
...
* stop_height (numeric, optional, default=0): The height limit to stop the rescanning. When timestamp is set this numeric value will be understood as a block UNIX timestamp.

* use_timestamp (boolean flag, optional, default=false): When present in the command the provided values will be treated as UNIX timestamps.
...
If `timestamp` is true, apart the previous rules, if any of the values is smaller than that of the genesis block (1231006505), the execution will also fail.
