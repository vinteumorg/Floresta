...
pub fn handle_descriptors_requests(
    ...
    for request in requests {
        ...
        descriptors.push(request.into_concrete_descriptors()?);
    }
...
impl DescriptorRequest {
    ...
    pub fn into_concrete_descriptors(self) -> Result<ConcreteDescriptor, DescriptorError> {
        let descriptor = Descriptor::<DescriptorPublicKey>::from_str(&self.desc)?;
        ...
        Ok(ConcreteDescriptor { descriptor, label, range, internal })
    }
}
...
mod tests {
    ...
    fn test_initial_wallet_build() {
        ...
        assert_eq!(
            resolve_descriptors_with(&w1_descriptor.0, |d| Ok(d.to_string())).unwrap(),
        ...
        assert_eq!(
                ...
                .0, |d| Ok(d.address(network).unwrap().to_string())).unwrap(),
            addresses
        );
        ...
        let casted_w1_addr = resolve_descriptors_with(&w1_descriptor.0, | d| Ok(d.address(network)?.script_pubkey())).unwrap();
    ...
    fn descriptor_counting() {
        ...
        assert!(resolved.len() == expected_count);
    }
