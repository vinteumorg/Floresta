...
/// The structs and types made for internal handling.
pub mod units {
    use core::cmp::max;
    use core::cmp::min;
    ...
    impl ConcreteDescriptor {
        /// Include the given inside the `DerivationRange` and return the new range.
        ///
        /// Wrapper around `DerivationRange::with_index`
        pub fn sum_index(&mut self, idx: u32) -> () {
            self.range = self.range.sum_index(idx);
        }

        pub fn sum_descriptors_index(to_sum: &mut Vec<ConcreteDescriptor>, idx: u32) -> () {
            for d in to_sum {
                d.sum_index(idx);
            }
        }

        pub fn how_many_descriptors_itll_yield(target: &[ConcreteDescriptor]) -> usize {
            let mut ret: usize = 0;
        ...
        pub fn resolve_descriptors(
            to_resolve: &[ConcreteDescriptor],
        ) -> Vec<Descriptor<DefiniteDescriptorKey>> {
            to_resolve
                .iter()
                .map(ConcreteDescriptor::resolve)
                .flatten()
                .collect()
        }

        pub fn resolve_descriptors_with<C, T>(
            to_resolve: &[ConcreteDescriptor],
            into: C,
        ...
        {
            let mut ret =
                Vec::<T>::with_capacity(Self::how_many_descriptors_itll_yield(to_resolve));
            for d in to_resolve {
    ...
    pub enum DerivationRangeCreationError {
        /// stop at cant be 0.
        StopAt,
        /// start at cant be 0. Use `DerivationRange::End` instead.
        StartAt,
    ...
    impl fmt::Display for DerivationRangeCreationError {
        fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
            match self {
                DerivationRangeCreationError::StopAt => {
                    write!(f, "stop at cannot be 0")
                }
                DerivationRangeCreationError::StartAt => {
                    write!(
                        f,
                        "start at cannot be 0. Use `DerivationRange::End` instead"
                    )
                }
                DerivationRangeCreationError::ProperRange => {
    ...
    impl DerivationRange {
        ...
        pub fn new_end(stop_at: u32) -> Result<DerivationRange, DerivationRangeCreationError> {
            if stop_at == 0 {
                return Err(DerivationRangeCreationError::StopAt);
            }
            Ok(Self::End(stop_at))
        }
        ...
        pub fn new_ranged(
            ...
            if start_at == 0 {
                return Err(DerivationRangeCreationError::StartAt);
            }

            if start_at >= stop_at {
                return Err(DerivationRangeCreationError::ProperRange);
            }
        ...
        /// Extends `stop_at` with the given.
        fn sum_index(&self, to_sum: u32) -> DerivationRange {
            match self {
                Self::End(e) => DerivationRange::End(to_sum + e),
                Self::Range(s, e) => DerivationRange::Range(s.clone(), to_sum + e),
            }
        }
    }
...
/// Converts the inserted addresses, descriptors and xpubs into compatible types so we can cache them using the wallet API.
pub fn convert_to_internal(
    ...
    for d in xpub_descriptor_request
        .into_iter()
...
mod tests {
    ...
    fn test_error_returning() {
        ...
        let result =
            DescriptorRequest::handle_descriptors_requests(vec![Default::default(), bad_req]);
    ...
    fn test_all_requests_processed() {
        ...
        let (descs, rescan) = DescriptorRequest::handle_descriptors_requests(reqs).unwrap();
    ...
    fn test_initial_wallet_build() {
        ...
        let w1_xpub = convert_to_internal(&[zpub], &[], &[], network, addresses_per_descriptor)
            .unwrap()
            .0;
        ...
        let w1_xpub_resolved = ConcreteDescriptor::resolve_descriptors(&w1_xpub);

        let w1_descriptor_resolved = ConcreteDescriptor::resolve_descriptors(&w1_descriptor);
        ...
        assert_eq!(
            ConcreteDescriptor::resolve_descriptors_with(&w1_descriptor, |d| Ok(d.to_string())).unwrap(),
        ...
        assert_eq!(
            ConcreteDescriptor::resolve_descriptors_with(&w1_descriptor, |d| Ok(d
                .address(network)
                .unwrap()
                .to_string()))
            .unwrap(),
        ...
        let casted_w1_addr = ConcreteDescriptor::resolve_descriptors_with(&w1_descriptor, |d| {
            Ok(d.address(network)?.script_pubkey())
        })
        .unwrap();
    ...
    fn descriptor_index_control() {
        ...
        let expected_count = ConcreteDescriptor::how_many_descriptors_itll_yield(&wallet);

        let resolved = ConcreteDescriptor::resolve_descriptors(&wallet);
        ...
        let mut converted = from_request.into_concrete_descriptors().unwrap();
        ...
        let resolved_converted = ConcreteDescriptor::resolve_descriptors(&converted);

        assert_eq!(resolved_converted.len(), 84usize);

        let guess = ConcreteDescriptor::how_many_descriptors_itll_yield(&converted);

        assert_eq!(guess, 84usize);

        ConcreteDescriptor::sum_descriptors_index(&mut converted, 1); // This should add 1 to each index.

        let sum_guess = ConcreteDescriptor::how_many_descriptors_itll_yield(&converted);
    ...
    fn test_initial_wallet_build_multisig_testnet() {
        ...
        let to_script = ConcreteDescriptor::resolve_descriptors_with(&w1_descriptor.0, |d| {
            Ok(d.address(network)?.to_string())
        })
        .unwrap();
