...
///
/// This bitmap represents which inputs the sender need in the proof.
...
impl Bitmap {
    ...
    ///
    /// Use `push_input` method to add inputs to the bitmap.
    pub fn new() -> Self {
        Self {
            bytes: Vec::new(),
            n_inputs: 0,
    ...
    pub fn push_input(&mut self, request: bool) {
        ...
        let bit_offset = current_n_inputs % u8::BITS;
        if bit_offset == 0 {
            self.bytes.push(request as u8);
            return;
        }

        let byte_n = current_n_inputs / u8::BITS;
        let byte = self.bytes.get_mut(byte_n as usize).expect("infallible: If this is not the first bit in a byte, it should already be here (see the if above)");
...
impl Decodable for UtreexoProof {
    fn consensus_decode<R: bitcoin::io::Read + ?Sized>(
        ...
        // Read the leaf data
        let n_leaf_data = VarInt::consensus_decode(reader)?.0 as usize;
        if n_leaf_data > MAX_INPUTS_PER_BLOCK {
            return Err(encode::Error::OversizedVectorAllocation {
                requested: n_leaf_data,
                max: MAX_INPUTS_PER_BLOCK,
            });
        }
...
mod utreexo_proof_tests {
    ...
    fn test_process_proof_utxo_order() {
        ...
        state
            .validate_block_no_acc(&block, height, leaf_data)
