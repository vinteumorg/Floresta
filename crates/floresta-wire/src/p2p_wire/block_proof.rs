...
///
/// Assuming that each UTXO needs a proof, with no everlaps of any kind, the maximum number of
...
impl Bitmap {
    ...
    pub fn push_input(&mut self, request: bool) {
        ...
        let current_n_inputs = self.n_inputs;
        self.n_inputs += 1;

        let bit_offset = current_n_inputs % BITS_IN_A_BYTE;
        if bit_offset == 0 {
            self.bytes.push(request as u8);
            return;
        }

        let byte_n = current_n_inputs / BITS_IN_A_BYTE;
        let byte = self.bytes.get_mut(byte_n).expect("infallible: If this is not the first bit in a byte, it should already be here (see the if above)");
...
impl Decodable for UtreexoProof {
    fn consensus_decode<R: bitcoin::io::Read + ?Sized>(
        ...
        if n_hashes > MAX_PROOF_HASHES {
            return Err(encode::Error::OversizedVectorAllocation {
                requested: n_hashes,
                max: MAX_PROOF_HASHES,
            });
        }
        ...
        if n_targets > MAX_INPUTS_PER_BLOCK {
            return Err(encode::Error::OversizedVectorAllocation {
                requested: n_targets,
                max: MAX_INPUTS_PER_BLOCK,
            });
        }
        ...
        for _ in 0..n_targets {
            ...
            targets.push(target.0); // FIXME: REVERT THESE TEST CHANGES
        }

        // Read the leaf data
        let n_leaf_datas = VarInt::consensus_decode(reader)?.0 as usize;
        if n_leaf_datas > MAX_INPUTS_PER_BLOCK {
            return Err(encode::Error::OversizedVectorAllocation {
                requested: n_leaf_datas,
                max: MAX_INPUTS_PER_BLOCK,
            });
        }
...
mod utreexo_proof_tests {
    ...
    fn test_process_proof_utxo_order() {
        fn setup_test_chain(
            ...
            let config = FlatChainStoreConfig::new("./db".into());
        ...
        let (del_hashes, leaf_datas) =
            process_proof(&proof.leaf_datas, &block.txdata, height, get_block_hash).unwrap();
        let r_proof = Proof {
        ...
        state
            .validate_block_no_acc(&block, height, leaf_datas)
        ...
        let (del_hashes, _utxos) =
            process_proof(&proof.leaf_datas, &invalid_txdata, height, get_block_hash).unwrap();
