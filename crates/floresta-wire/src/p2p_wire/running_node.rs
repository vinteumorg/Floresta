    ...
    Chain: BlockchainInterface + UpdatableChainstate + 'static,
{
    ...
    pub fn get_handle(&self) -> Arc<NodeInterface> {
        self.node_module.user_requests.clone()
    }
    ...
    fn handle_get_peer_info(&self) {
        ...
        self.node_module.user_requests.send_answer(
        ...
        for request in timed_out {
            ...
            match request {
                ...
                InflightRequests::UserRequest(req) => {
                    self.node_module.user_requests.send_answer(req, None);
                }
    ...
    pub async fn run(
        ...
        let mut ibd = UtreexoNode {
            core: self.core,
            node_module: ChainSelector::default(),
        };
        ...
        if self.config.backfill && startup_tip == 0 {
            let end = self.core.chain.get_validation_index().unwrap();
            let chain = self
                .chain
        ...
        loop {
            ...
            periodic_job!(
                ...
                self.node_module.last_address_rearrange,
    ...
    fn get_peer_score(&self, peer: PeerId) -> u32 {
        let mut score = 0;
        for block in self.node_module.last_invs.keys() {
            if self.node_module.last_invs[block].1.contains(&peer) {
                score += 1;
            }
        }
        ...
        match notification {
            NodeNotification::FromPeer(peer, message) => match message {
                ...
                PeerMessages::BlockFilter((hash, filter)) => {
                    ...
                    if let Some(filters) = self.core.block_filters.as_ref() {
                        ...
                        while let Some(filter) =
                            self.node_module.inflight_filters.remove(&(current_height))
                        {
                        ...
                        if self.last_filter == current_hash
                            && self.node_module.inflight_filters.is_empty()
                        {
