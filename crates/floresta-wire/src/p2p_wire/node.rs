...
/// have available at the time of opening or accepting the connection.
///
    ...
    WireError: From<Chain::Error>,
{
    ...
    pub(crate) async fn maybe_open_connection_with_added_peers(&mut self) -> Result<(), WireError> {
        ...
        if self.added_peers.len() >= T::MAX_FULL_RELAY_PEERS {
            return Ok(());
        }
    ...
    /// (special connection types that is used to learn about good peers, but are not kept
    /// after handshake), [ConnectionKind::OutboundFullRelay] and [ConnectionKind::BlockRelayOnly]
    /// (connections that are kept after the handshake, the first could ask for any
    /// messages and the second ask only for blocks) and [ConnectionKind::Extra] (a type
    /// used when we have a stale tip).
    ///
