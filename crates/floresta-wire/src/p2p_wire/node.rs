...
pub struct UtreexoNode<Chain: BlockchainInterface + UpdatableChainstate, NodeModule> {
    pub(crate) core: NodeCommon<Chain>,
    pub(crate) node_module: NodeModule,
}
    ...
    Chain: BlockchainInterface + UpdatableChainstate + 'static,
{
    ...
    pub(crate) async fn handle_peer_ready(
        ...
        if let Some(peer_data) = self.core.peers.get_mut(&peer) {
    ...
    pub(crate) async fn increase_banscore(
        ...
        let Some(peer) = self.core.peers.get_mut(&peer_id) else {
            return Ok(());
        };
        ...
        let is_missbehaving = peer.banscore >= self.core.max_banscore;
    ...
    pub(crate) async fn init_peers(&mut self) -> Result<(), WireError> {
        let anchors = self.core.address_man.start_addr_man(
            self.datadir.clone(),
    ...
    fn get_required_services(&self) -> ServiceFlags {
        let required_services = self.node_module.get_required_services();
