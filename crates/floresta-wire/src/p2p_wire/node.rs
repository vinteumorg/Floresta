    ...
    Chain: BlockchainInterface + UpdatableChainstate + 'static,
{
    ...
    /// This means the node is marked as a "manually added node" for future connection attempts oes not directly establish a connection to the node.
    ...
    pub async fn handle_addnode_add_peer(
        ...
        let response = match self
            .added_peers
            .iter()
            .any(|info| address == info.address && info.port == port)
        {
            true => {
                warn!("Peer {addr}:{port} already exists");
    ...
    pub async fn handle_addnode_remove_peer(
        &mut self,
        responder: oneshot::Sender<NodeResponse>,
        addr: IpAddr,
        port: u16,
        ...
        let index = self
            .added_peers
            .iter()
            .position(|info| info.address == address && info.port == port);

        let response = if let Some(peer_id) = index {
            self.added_peers.remove(peer_id);
            info!("Peer {addr}:{port} removed");
    ...
    /// Handles addnode onetry requests, connecting to the node and his will try to connect to the given address and port.
    ...
    pub async fn handle_addnode_onetry_peer(
        ...
        let peer_already_exists = self
            .peers
            .iter()
            .any(|(_, peer)| peer.address == addr && peer.port == port);

        if peer_already_exists {
            warn!("Peer {addr}:{port} already exists");
            let response = NodeResponse::Onetry(false);
            responder
                .send(response)
                .map_err(|_| WireError::ResponseSendError)?;
            return Ok(());
        ...
        let response = NodeResponse::Onetry(true);
    ...
    pub(crate) async fn perform_user_request(
        ...
        let req = match user_req {
            ...
            UserRequest::Add((addr, port, v2transport)) => {
                ...
                let _ = self
                    .handle_addnode_add_peer(responder, addr, port, transport_protocol)
                    .await
                    .map_err(|_| WireError::FailedToAddPeer);
                return;
            }
            UserRequest::Remove((addr, port)) => {
                let _ = self
                    .handle_addnode_remove_peer(responder, addr, port)
                    .await
                    .map_err(|_| WireError::FailedToRemovePeer);
                return;
            }
            UserRequest::Onetry((addr, port, v2transport)) => {
                ...
                let _ = self
                    .handle_addnode_onetry_peer(responder, addr, port, transport_protocol)
                    .await
                    .map_err(|_| WireError::FailedToConnectToPeer);
                return;
            }
        };
    ...
    pub(crate) async fn maybe_open_connection_with_added_peers(&mut self) -> Result<(), WireError> {
        ...
        let to_addr_v2 = |ip: IpAddr| match ip {
            IpAddr::V4(addr) => AddrV2::Ipv4(addr),
            IpAddr::V6(addr) => AddrV2::Ipv6(addr),
        };
        ...
        for added_peer in self.added_peers.clone() {
            let matching_peer = self.peers.values().find(|peer| {
                to_addr_v2(peer.address) == added_peer.address && peer.port == added_peer.port
            });
