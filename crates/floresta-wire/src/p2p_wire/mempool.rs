...
impl Mempool {
    ...
    ///
    /// This method will remove all transactions that is in the block from mempool and
    /// return the txids that were included in the block.
    ...
    pub fn accept_to_mempool(
        ...
        // Makes sure our mempool won't grow too much if we have this transaction in it
        //
        // TODO(@davidson): Proper eviction code
        if self.mempool_size + tx_size > self.max_mempool_size {
            return Err(AcceptToMempoolError::MemoryUsageTooHigh);
        }

        // Make some context-free validation for this transaction
