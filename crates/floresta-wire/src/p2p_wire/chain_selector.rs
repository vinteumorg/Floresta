    ...
    Chain: BlockchainInterface + UpdatableChainstate + 'static,
{
    ...
    async fn handle_headers(
        ...
        for header in headers.iter() {
            if let Err(e) = self.chain.accept_header(*header) {
                ...
                self.core.address_man.update_set_state(
    ...
    async fn empty_headers_message(&mut self, peer: PeerId) -> Result<(), WireError> {
        match self.node_module.state {
            ChainSelectorState::DownloadingHeaders => {
                ...
                self.node_module.state = ChainSelectorState::LookingForForks(Instant::now());
                self.node_module.done_peers.insert(peer);
            }
            ChainSelectorState::LookingForForks(_) => {
                self.node_module.done_peers.insert(peer);
                for peer in self.core.peer_ids.iter() {
                    // at least one peer haven't finished
                    if !self.node_module.done_peers.contains(peer) {
                        return Ok(());
                    }
                }

                if let Some(assume_utreexo) = self.core.config.assume_utreexo.as_ref() {
                    self.node_module.state = ChainSelectorState::Done;
    ...
    async fn request_headers(&mut self, tip: BlockHash, peer: PeerId) -> Result<(), WireError> {
        ...
        let peer = self.node_module.sync_peer;
        self.inflight
    ...
    async fn check_for_timeout(&mut self) -> Result<(), WireError> {
        ...
        for peer in peers {
            self.core.peers.entry(peer).and_modify(|e| {
    ...
    pub async fn run(&mut self, stop_signal: Arc<RwLock<bool>>) -> Result<(), WireError> {
        ...
        loop {
            ...
            if let ChainSelectorState::LookingForForks(start) = self.node_module.state {
            ...
            if self.node_module.state == ChainSelectorState::CreatingConnections {
                // If we have enough peers, try to download headers
                if !self.peer_ids.is_empty() {
                    let new_sync_peer = rand::random::<usize>() % self.peer_ids.len();
                    self.node_module.sync_peer = *self.peer_ids.get(new_sync_peer).unwrap();
                    try_and_log!(
                        self.request_headers(
                            self.chain.get_best_block()?.1,
                            self.node_module.sync_peer
                    ...
                    self.node_module.state = ChainSelectorState::DownloadingHeaders;
                }
            }

            // We downloaded all headers in the most-pow chain, and all our peers agree
            // this is the most-pow chain, we're done!
            if self.node_module.state == ChainSelectorState::Done {
    ...
    async fn find_accumulator_for_block_step(
        ...
        for peer_id in self.core.peer_ids.iter() {
        ...
        if let Some(NodeNotification::FromPeer(peer, message)) = notification {
            match message {
                ...
                PeerMessages::Disconnected(idx) => {
                    if peer == self.node_module.sync_peer {
                        self.node_module.state = ChainSelectorState::CreatingConnections;
                    }
