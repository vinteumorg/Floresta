...
//! existed. However, although this is a pretty manageble number, we can safely get rid of some
...
impl ChainStore for FlatChainStore {
    ...
    fn save_roots_for_block(&mut self, roots: Vec<u8>, height: u32) -> Result<(), Self::Error> {
        ...
        let validation_index = self
            ...
            .map(|h| h.try_height().unwrap_or(0))
            .ok_or(FlatChainstoreError::NotFound)?;
