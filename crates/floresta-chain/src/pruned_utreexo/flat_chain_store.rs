...
impl BlockIndex {
    ...
    unsafe fn hash_map_find_pos(
        ...
        loop {
            ...
            // Read the index entry from the bucket
            let index = (*entry_ptr).index();

            // Get the block header by the height
            let header = get_block_header_by_height(*entry_ptr)?;

            // If the hash matches the block hash, return the entry
            if header.hash == block_hash {
                return Ok(Entry::Occupied(entry_ptr));
            }

            // If the index is empty, return the position where the entry would be added
            if index == 0 {
...
impl FlatChainStore {
    ...
    pub fn create_chain_store(config: FlatChainStoreConfig) -> Result<Self, FlatChainstoreError> {
        ...
        let header_size = headers_size * size_of::<DiskBlockHeaderAndHash>();
        let headers =
    ...
    pub fn new(config: FlatChainStoreConfig) -> Result<Self, FlatChainstoreError> {
        ...
        let headers_size = metadata.headers_file_size * size_of::<DiskBlockHeaderAndHash>();
        let fork_size = metadata.fork_file_size * size_of::<DiskBlockHeaderAndHash>();
    ...
    unsafe fn get_header_by_height(
        ...
        let header = self.headers.as_ptr().wrapping_add(pos) as *mut DiskBlockHeaderAndHash;
    ...
    unsafe fn write_header_to_storage(
        ...
        let ptr = self.headers.as_ptr().wrapping_add(offset) as *mut DiskBlockHeaderAndHash;
