...
///
/// Worse case, we have 64 roots, each with 32 bytes, and a 64 byte integer for the number of
...
pub struct FlatChainStoreConfig {
    ...
    ///
    /// This index holds our map from block hashes to block heights. We use a open-addressing hash
...
///
/// This entry will tell whether a given bucket is occupied or not, and if it is, what is the
...
pub struct FlatChainStore {
    ...
    cache: UnsafeCell<LruCache<BlockHash, DiskBlockHeader>>,
}
...
impl FlatChainStore {
    ...
    #[must_use]
    #[doc(hidden)]
    unsafe fn get_cache_mut(&self) -> &mut LruCache<BlockHash, DiskBlockHeader> {
...
impl ChainStore for FlatChainStore {
    ...
    fn load_roots(&self) -> Result<Option<Vec<u8>>, Self::Error> {
        unsafe { self.get_acc_inner().map(Some) }
    }

    fn get_header(&self, block_hash: &BlockHash) -> Result<Option<DiskBlockHeader>, Self::Error> {
        let cache = unsafe { self.get_cache_mut() };
...
mod tests {
    ...
    fn test_index() {
        let store = get_test_chainstore();
        println!("{:?}", store.block_index.index_size);
