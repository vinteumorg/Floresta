...
impl BlockIndex {
    ...
    unsafe fn set_index_for_hash(
        ...
        match pos {
            ...
            IndexBucket::Empty(pos) | IndexBucket::Occupied(pos) => pos.write(index),
        }
    ...
    unsafe fn get_index_for_hash(
        ...
        match self.hash_map_find_pos(hash, get_header_by_index)? {
            ...
            IndexBucket::Occupied(index) => Ok(Some(*index)),
        }
    }
    ...
    unsafe fn hash_map_find_pos(
        ...
        // Reads may loop forever if the index is full
        if self.occupied == self.index_size {
            return Err(FlatChainstoreError::IndexIsFull);
        }
        ...
        loop {
            ...
            let entry_ptr = base_ptr.wrapping_add(byte_offset) as *mut Index;
...
impl FlatChainStore {
    ...
    pub fn create_chain_store(config: FlatChainStoreConfig) -> Result<Self, FlatChainstoreError> {
        ...
        let header_size = headers_size * size_of::<HashedDiskHeader>();
        let headers =
            unsafe { Self::init_file(&PathBuf::from(&headers_file), header_size, file_mode)? };
        ...
        let fork_headers_size = fork_size * size_of::<HashedDiskHeader>();
    ...
    pub fn new(config: FlatChainStoreConfig) -> Result<Self, FlatChainstoreError> {
        ...
        let metadata_file = match unsafe {
            Self::init_file(
                &PathBuf::from(&metadata_file),
                size_of::<Metadata>(),
                file_mode,
        ...
        let headers_file = format!("{}/headers.bin", config.path);
        let fork_file = format!("{}/fork_headers.bin", config.path);
        let index_size = metadata.block_index_size * size_of::<u32>();
        let headers_size = metadata.headers_file_size * size_of::<HashedDiskHeader>();
        let fork_size = metadata.fork_file_size * size_of::<HashedDiskHeader>();
    ...
    unsafe fn get_header_by_height(
        ...
        let offset = height as usize * size_of::<HashedDiskHeader>();

        let header = self.headers.as_ptr().wrapping_add(offset) as *mut HashedDiskHeader;
    ...
    unsafe fn get_block_header_by_index(
        ...
        match index.is_main_chain() {
            ...
            false => {
                let offset = index.index() as usize * size_of::<HashedDiskHeader>();
                let header =
                    self.fork_headers.as_ptr().wrapping_add(offset) as *mut HashedDiskHeader;
    ...
    unsafe fn write_header_to_storage(
        ...
        let offset = size_of::<HashedDiskHeader>() * header.height().unwrap() as usize;
        let ptr = self.headers.as_ptr().wrapping_add(offset) as *mut HashedDiskHeader;

        *ptr = HashedDiskHeader {
            header,
    ...
    unsafe fn save_fork_block(&self, header: DiskBlockHeader) -> Result<(), FlatChainstoreError> {
        ...
        let offset = size_of::<HashedDiskHeader>() * fork_blocks as usize;
        let ptr = self.fork_headers.as_ptr().wrapping_add(offset) as *mut HashedDiskHeader;
        let block_hash = header.block_hash();

        ptr.write(HashedDiskHeader {
            header,
            hash: block_hash,
        });
        ...
        index.set_fork();
        self.block_index
            .set_index_for_hash(block_hash, index, |height| {
