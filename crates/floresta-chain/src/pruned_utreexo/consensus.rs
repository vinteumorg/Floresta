...
impl Consensus {
    ...
    pub fn verify_block_transactions(
        ...
        for (n, transaction) in transactions.iter().enumerate() {
            ...
            for output in transaction.output.iter() {
                ...
                Self::validate_script_size(&output.script_pubkey, false).map_err(|err| {
                    TransactionError {
                        txid: transaction.compute_txid(),
                        error: err,
                    }
            ...
            for input in transaction.input.iter() {
                let txo = Self::get_utxo(input, &utxos).map_err(|err| TransactionError {
                    txid: transaction.compute_txid(),
                    error: err,
                })?;

                in_value += txo.value.to_sat();
                // We need to know if this input spends a taproot output
                let taproot_spend = txo.script_pubkey.is_p2tr();

                Self::validate_script_size(&input.script_sig, taproot_spend).map_err(|err| {
                    TransactionError {
                        txid: transaction.compute_txid(),
                        error: err,
                    }
            ...
            // Checks vBytes / WU validation
            // Sum the transaction weights to get the block weight
    ...
    fn validate_script_size(
        ...
        // The maximum script size is 10,000 bytes (https://github.com/bitcoin/bitcoin/blob/v28.0/src/script/script.h#L39)
        // Taproot spend scripts are the exception, which are only bound by the maximum block size
        if script.len() > 10_000 && !taproot_spend {
...
mod tests {
    ...
    fn test_validate_script_size() {
        ...
        assert!(Consensus::validate_script_size(&small_script, false).is_ok());
        assert!(Consensus::validate_script_size(&large_script, false).is_err());

        // Large script is valid if this is a taproot spend
        assert!(Consensus::validate_script_size(&large_script, true).is_ok());
    }
