...
impl Consensus {
    ...
    fn validate_script(
        ...
        if Self::validate_script_size(tx, utxo_map).is_ok() {
            return match tx.verify_with_flags(
                |outpoint| match utxo_map.get(outpoint) {
                    Some(obj) => Some(obj.txout.clone()),
                    _ => None,
                },
                flags,
            ) {
    ...
    fn validate_locktime(
        ...
        if input.sequence.is_relative_lock_time() {
            ...
            if input.sequence.is_time_locked() {
                ...
                let time_lock = ((input.sequence.0 << 16) >> 16) * 512 as u32;
    ...
    fn validate_script_size(
        ...
        for input in tx.input.iter() {
            ...
            if (scrip_sig_size > 1650 || scrip_sig_size < 2) && !is_taproot {
...
mod tests {
    ...
    fn test_validate_locktime_relative_time() {
        ...
        assert!(Consensus::validate_locktime(&tx.input[0], &tx, &out_map, 0 as u32, mtp).is_ok());
        ...
        assert!(Consensus::validate_locktime(&tx.input[0], &tx, &out_map, 0 as u32, mtp).is_err());
    }
}
