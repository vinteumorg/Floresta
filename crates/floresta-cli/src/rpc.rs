...
pub trait FlorestaRPC {
    ...
    fn list_descriptors(&self) -> Result<ListDescriptorRes>;
    ...
    fn delete_descriptors(&self, id: Vec<DescriptorId>, pull: bool) -> Result<DeleteDescriptorRes>;
}
...
impl<T: JsonRPCClient> FlorestaRPC for T {
    ...
    fn rescanblockchain(
        ...
        pub const GENESIS_TIMESTAMP: u32 = 1231006505; // The u32 representation of 03 Jan 2009, 18:15:05. The salvation of human kind.

        let start_height = start_height.unwrap_or(0u32);

        let stop_height = stop_height.unwrap_or(0u32);

        // We need to check some constraints before sending the request.
        // The same constraints are checked server side to maintain api consistency.
        if stop_height != 0 && start_height >= stop_height {
            ...
            return Err(Error::InvalidRescanVal);
        }

        let now = self.get_blockchain_info()?.latest_block_time; // This is better than using any std implementation right ?

        // no need to check start-height as i already know that its less than stop-height for this check.
        if stop_height > now {
            return Err(Error::InvalidRescanVal);
        }

        // When timestamp values are inserted we need to check some contraints only in case
        // theyre non zero.
        if use_timestamp
            && (stop_height != 0 && start_height != 0)
            && start_height < GENESIS_TIMESTAMP
        {
            // We dont need to check stop height because at this point it is atleast greater than start_height
            return Err(Error::InvalidRescanVal);
        }
    ...
    fn list_descriptors(&self) -> Result<ListDescriptorRes> {
        let res: Vec<ConcreteDescriptor> =
            serde_json::from_value(self.call("listdescriptors", &[])?)?;
        Ok(ListDescriptorRes(
            res.iter()
                .map(|d| VerboseDescriptor {
                    desc: d.descriptor.to_string(),
                    internal: d.internal,
                    label: d.label.clone(),
                    hash: d.get_hash().to_string(),
                })
                .collect::<Vec<VerboseDescriptor>>(),
        ))
    }
    ...
    fn delete_descriptors(
        ...
        self.call(
            "deletedescriptors",
            &[serde_json::to_value(ids)?, Value::Bool(pull)],
        )
    }
}
