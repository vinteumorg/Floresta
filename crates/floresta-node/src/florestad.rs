...
impl Florestad {
    ...
    pub fn setup_logger(
        ...
        // Set Tokio's and the runtime's log level
        // to `trace`, if the `tokio-console` feature is enabled.
        ...
        let base_filter_stdout = log_filter.clone();
        let fmt_layer_stdout = log_to_stdout.then(|| {
            fmt::layer()
                ...
                .with_target(true)
                .with_level(true)
                .with_filter(base_filter_stdout)
        });

        // Formatting [`Layer`] for logs destined to the log file.
        let mut guard = None;
        let base_filter_logfile = log_filter.clone();
        let fmt_layer_logfile = log_to_file.then(|| {
            ...
            fmt::layer()
                ...
                .with_target(true)
                .with_level(true)
                .with_filter(base_filter_logfile)
        });

        // Create a [`Registry`] with the base [`EnvFilter`].
