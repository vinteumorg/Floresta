...
//!
//! The watch-only wallet doesnt store any private keys so [`DescriptorUnit`] holds a [`Descriptor<DescriptorPublicKey>`] which should satisfy
//! all use cases that floresta might need, [`DescriptorUnit`] also holds some metadata such as a `DerivationRange` that accepts operations to extend and sum indices.
//!
//! Everything in this module works around [`DescriptorUnit`] and to work with them. Heres a short description about each struct and what you should do with them.
//!
...
//! any kind of descriptor as an input, besides the invalid ones since the validation for correctnessonly happens in [`DescriptorRequest::into_unit`], since this should be the only
//! way to produce [`DescriptorUnit`]s, and this facilitates error handling.
//!
...
impl DescriptorRequest {
    ...
    ///
    /// The return is a Vec of it because a [`DescriptorRequest`] may yield more than one [`DescriptorUnit`].
...
impl DescriptorUnit {
    ...
    /// Resolve the descriptors off the given array, deriving all its children.
    ///
    /// Similar to [`DescriptorUnit::resolve`] but for an array.
    pub fn resolve_descriptors(
        to_resolve: &[DescriptorUnit],
    ) -> Vec<Descriptor<DefiniteDescriptorKey>> {
        to_resolve
            .iter()
            .flat_map(DescriptorUnit::resolve)
            .collect()
    }

    /// Same as [`DescriptorUnit::resolve_descriptors`] but accepts an convert function thats applied while the iteration is going.
    ///
    /// Similar to [`DescriptorUnit::resolve_with`] but for an array.
    ...
    {
        let mut ret = Vec::<T>::with_capacity(Self::arr_yield_count(to_resolve));
        for d in to_resolve {
    ...
    pub fn arr_yield_count(target: &[DescriptorUnit]) -> usize {
        let mut ret: usize = 0;
    ...
    /// Similar to [`DescriptorUnit::resolve`] but you can pass a function to consume the resolved [`Descriptor<DefiniteDescriptorKey>`], converting them to something like an Address.
    ///
...
mod tests {
    ...
    fn descriptor_index_control() {
        ...
        let expected_count = DescriptorUnit::arr_yield_count(&wallet);

        let resolved = DescriptorUnit::resolve_descriptors(&wallet);
        ...
        let resolved_converted = DescriptorUnit::resolve_descriptors(&converted);

        assert_eq!(resolved_converted.len(), 84usize);
        let guess = DescriptorUnit::arr_yield_count(&converted);

        assert_eq!(guess, 84usize);

        DescriptorUnit::arr_sum_index(&mut converted, 1); // This should add 1 to each index.
        let sum_guess = DescriptorUnit::arr_yield_count(&converted);
