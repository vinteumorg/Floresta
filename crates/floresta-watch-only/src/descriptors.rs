...
//! any kind of descriptor as an input, besides the invalid ones since the validation for correctness only happens in [`DescriptorRequest::into_unit`], since this should be the only
...
impl DescriptorUnit {
    ...
    {
        let mut ret = Vec::<T>::with_capacity(Self::batch_yield_count(to_resolve));
        for d in to_resolve {
