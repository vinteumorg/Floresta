...
impl<Blockchain: RpcChain> RpcImpl<Blockchain> {
    ...
    async fn import_descriptors(&self, requests: Vec<DescriptorRequest>) -> Result<bool> {
        ...
        // Im sure theres a smarter way to do these checks below but im not the one that will figure it out. TODO™.
        ...
        if self.chain.is_in_ibd() {
            ...
            match rescan_timestamp {
                RescanRequest::Full => self.rescan_blockchain(None, None, false, Some(RescanConfidence::Raw)).await, 
                RescanRequest::SpecifiedTime(time) => {
                    self.rescan_blockchain(Some(time), None, true, Some(RescanConfidence::Medium)).await
                }
                RescanRequest::Now => {
    ...
    async fn rescan_blockchain(
        ...
        let mut start_height = start_height.unwrap_or(0u32); // unwraps to default so we can deal with the values.
        let mut stop_height = stop_height.unwrap_or(0u32); // Since 0 as stop doesnt make any sense we use it as disable value.
        ...
        if use_timestamp {
            ...
            start_height = self
                .get_block_height_by_timestamp(start_height, &confidence)
                .await?;

            stop_height = self
                .get_block_height_by_timestamp(stop_height, &RescanConfidence::Raw)
                .await?;
        } else {
            let (tip, _) = self
                .chain
