...
pub enum JsonRpcError {
    ...
    /// Raised if when the rescanblockchain command, with the timestamp flag activated, contains some timestamp thats less than the genesis one.
    InvalidTimestamp,
}
...
impl Display for JsonRpcError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            JsonRpcError::InvalidTimestamp => write!(f, "Invalid timestamp, ensure thats between the genesis and the tip."),
