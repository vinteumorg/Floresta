...
pub struct Config {
    ...
    /// Wheter to enable TLS for the Electrum Server.
    ...
    pub fn start(&self) {
        ...
        if self.config.enable_electrum_tls {
            ...
            if self.config.generate_cert {
                ...
                // Define file paths
                let tls_key_path = format!("{}tls/key.pem", &data_dir);
                let tls_cert_path = format!("{}tls/cert.pem", &data_dir);
            ...
            let tls_config = Self::create_tls_config(self, &data_dir)
            ...
            let tls_listener = match block_on(TcpListener::bind(electrum_addr)) {
                Ok(listener) => Arc::new(listener),
                Err(_) => {
                    error!("Failed to bind Electrum Server. Something is already bound to {electrum_addr}");
                    std::process::exit(1);
                }
            };
