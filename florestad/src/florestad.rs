    ...
    fn resolve_hostname(hostname: &str, default_port: u16) -> Result<SocketAddr, FlorestadError> {
        ...
        let sock = match ip {
            ...
            Err(_) => {
                ...
                // We can safely use an expect here since the
                // first element of the split iterator is always
                // `Some` (even if there was no pattern matched)
                let hostname = split.next().expect("The provided ip address is invalid");
