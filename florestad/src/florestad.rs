...
pub struct Config {
    ...
    /// Whether we should write logs to `stdio`.
...
impl Florestad {
    ...
    pub fn start(&self) {
        ...
        // Instantiate the Electrum Server instance.
        ...
        let electrum_port: u16 = Self::get_electrum_port(self.config.network, false);
        ...
        let electrum_addr: SocketAddr = self
            ...
            .electrum_address
            .clone()
            .map(|addr| Self::resolve_hostname(&addr, electrum_port))
            .unwrap_or(
        ...
        if self.config.enable_electrum_tls {
            ...
            let electrum_port_tls: u16 = Self::get_electrum_port(self.config.network, true);
            ...
            let electrum_addr_tls: SocketAddr = self
                ...
                .clone()
                .map(|addr| Self::resolve_hostname(&addr, electrum_port_tls))
                .unwrap_or(
                    format!("0.0.0.0:{electrum_port_tls}")
                        .parse()
                        .expect("Hardcoded address"),
                );
            ...
            let tls_config = match Self::create_tls_config(self, &data_dir) {
                Ok(config) => config,
                Err(_) => {
                    error!("Failed to create TLS configuration. You must create the certificate manually or pass the --generate-cert flag");
                    std::process::exit(1)
                }
            };
