...
impl Florestad {
    ...
    fn resolve_hostname(hostname: &str, default_port: u16) -> Result<SocketAddr, FlorestadError> {
        ...
        let sock = match ip {
            ...
            Err(_) => {
                ...
                let hostname = split.next().unwrap_or_else(|| {
                    // If we cannot parse the hostname, let's use the default one like 127.0.0.1
                    let default_host = "127.0.0.1";
                    debug!("Using default hostname: {default_host}");
                    default_host
                });
    ...
    pub async fn start(&self) -> Result<(), FlorestadError> {
        ...
        // TODO handle error better, it should hanldle the traited error
        self.setup_wallet(&data_dir, &mut wallet)
            .map_err(|e| FlorestadError::CouldNotSetupWallet(format!("{e}")))?;
        ...
        let proxy = self
            ...
            .map_or_else(
                || {
                    let default_proxy = "socks5://127.0/0.1:9050".to_string();
                    debug!("No proxy provided, using default proxy: {default_proxy}");
                    default_proxy
                        .parse::<SocketAddr>()
                        .map_err(FlorestadError::InvalidIpAddress)
                },
                |addr| Self::resolve_hostname(addr, 9050),
            )
            .ok();
                ...
                self.config
                    ...
                    .map_or_else(
                        || {
                            ...
                            let default_address = "127.0.1:8332".to_string();
                            default_address
                                .parse::<SocketAddr>()
                                .map_err(FlorestadError::InvalidIpAddress)
                        },
                        |addr| Self::resolve_hostname(addr, 8332),
                    )
                    .ok(),
        ...
        let electrum_addr: SocketAddr = self.config.electrum_address.as_ref().map_or_else(
            || {
                format!("127.0.0.1:{default_electrum_port}")
                    .parse::<SocketAddr>()
                    .map_err(FlorestadError::InvalidIpAddress)
            },
            |addr| Self::resolve_hostname(addr, default_electrum_port),
        )?;
        ...
        if self.config.enable_electrum_tls {
            ...
            let electrum_addr_tls: SocketAddr =
                self.config.electrum_address_tls.as_ref().map_or_else(
                    || {
                        format!("127.0.0.1:{default_electrum_port_tls}")
                            .parse::<SocketAddr>()
                            .map_err(FlorestadError::InvalidIpAddress)
                    },
                    |addr| Self::resolve_hostname(addr, default_electrum_port_tls),
                )?;
    ...
    fn setup_wallet<D: AddressCacheDatabase>(
        ...
        for descriptor in setup.descriptors {
            ...
            let is_cached = wallet
                .is_cached(&descriptor)
                .map_err(|e| FlorestadError::CouldNotObtainWalletCache(format!("{e}")))?;

            if !is_cached {
                wallet
                    .push_descriptor(&descriptor)
                    .map_err(|e| FlorestadError::CouldNotPushDescriptor(format!("{e}")))?;
            }
        }
