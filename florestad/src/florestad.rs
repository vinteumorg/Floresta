...
impl Florestad {
    ...
    pub async fn start(&self) -> Result<(), FlorestadError> {
        ...
        let assume_valid = self
            ...
            .transpose()
            .map_err(FlorestadError::InvalidAssumedValidValue)?;
        ...
        let assume_utreexo = match (pow_fraud_proofs, self.config.assume_utreexo) {
            (false, true) => floresta_chain::ChainParams::get_assume_utreexo(self.config.network)
                .map_or_else(
                    |err| {
                        let e = FlorestadError::NetworkAlreadyValidatedAsSupported(err);
                        warn!("{e}");
                        None
                    },
                    |value| {
                        info!("Using assume-utreexo value {value:?}");
                        Some(value)
                    },
                ),
            _ => None,
        ...
        let proxy = self.config.proxy.as_ref().map(|host| {
            host.parse::<SocketAddr>()
                .unwrap_or_else(|_| Self::resolve_hostname(host, 9050))
        });
    ...
    fn data_dir_path(config: &Config) -> String {
        ...
        match config.network {
            ...
            _ => {
                ...
                error!("This network is not supported: {network}");
                exit(1);
            }
        }
    ...
    fn load_chain_store(data_dir: String) -> ChainStore {
        ...
        match ChainStore::new(config) {
            Ok(db) => db,
            Err(e) => {
                error!("Failure while creating chainstate: {e:?}");
                exit(1);
            }
        }
    }
    ...
    fn load_chain_state(
        ...
        let db = match ChainStore::new(data_dir.clone()) {
            Ok(db) => db,
            Err(e) => {
                error!("Could not read db: {e:?}");
                exit(1);
            }
        };
    ...
    fn load_wallet(data_dir: &String) -> Result<AddressCache<KvDatabase>, FlorestadError> {
        let database = KvDatabase::new(data_dir.to_owned())
            .map_err(FlorestadError::CannotObtainALockOnDataDir)?;
        ...
        for descriptor in setup.descriptors {
            ...
            if !is_cached {
                wallet
                    ...
                    .map_err(|e| FlorestadError::CouldNotPushDescriptorToWallet(format!("{e}")))?;
            }
        }
