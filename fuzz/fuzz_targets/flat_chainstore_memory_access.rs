...
/// Creates a unique temporary directory for each fuzzing run.
/// Uses process ID to ensure uniqueness across concurrent runs.
fn create_temp_dir() -> PathBuf {
    let rand_id = format!("fuzz-{}", std::process::id());
    let temp_dir = std::env::temp_dir().join("floresta-fuzz").join(rand_id);
...
fuzz_target!(|data: &[u8]| {
    ...
    // Create a temporary directory for our chainstore with small sizes
    // to stress memory access patterns
    ...
    if let Ok(mut store2) = FlatChainStore::new(config2) {
