...
OPTIONS=$(getopt -o x:d:n:t:p:C:z:v:uscUNh --long xpub:,desc:,network:,tag:,proxy:,connect:,zmq-address:,assume-valid:,assume-utreexo,ssl,cfilters,uninstall,non-interactive,help -n "$0" -- "$@")
if [ $? -ne 0 ]; then
  show_usage
  exit 1
fi
...
interactive_setup() {
  ...
  case $choice in
    ...
    1) interactive_select_proxy ;;
    2) interactive_connect ;;
    3) interactive_zeromq ;;
    4) interactive_cfilters ;;
    5) interactive_ask_assume_valid ;;
    6) interactive_assume_utreexo ;;
    7) interactive_ssl ;;
    8) interactive_ask_for_add_xpubs ;;
    9) interactive_ask_for_add_descriptors ;;
    R) interactive_review ;;
    *)
      clear
      exit 0
      ;;
...
interactive_zeromq() {
  local zeromq_regex="^((http|https):\/\/)?((([0-9]{1,3}\.){3}[0-9]{1,3})|\[?[a-fA-F0-9:]+\]?):[0-9]{1,5}$"

  while true; do
    # Prompt user for zeromq input
    zeromq=$(dialog --title "Floresta-Installer (Set zeromq)" \
                  --inputbox "Provide a zeromq address (Optional: http://domain:port, https://domain:port, IPv4:port or IPv6:port)" \
                  10 60 \
                  3>&1 1>&2 2>&3)

    # Check if user pressed Cancel or ESC
    if [ $? -ne 0 ]; then
      dialog --title "Floresta-Installer" --msgbox "❌ ZeroMQ input canceled. Returning to setup menu." 8 45
      interactive_setup
      return 1
    fi
    ...
    if [[ "$zeromq" =~ $zeromq_regex ]]; then
      dialog --title "Floresta-Installer" --msgbox "✅ ZeroMQ set successfully: $zeromq" 10 60
      interactive_setup
      return 0
    else
...
interactive_ask_assume_valid() {
  if dialog --title "Floresta-Installer" --yesno "Do you want to use 'assume-valid'?\n\nThis option assumes blocks before this height are valid. We still verify amounts, UTXOs and rewards, but skip script validation." 15 60; then
