...
OPTIONS=$(getopt -o x:d:n:t:p:C:z:v:f:usUNh --long xpub:,desc:,network:,tag:,proxy:,connect:,zmq-address:,assume-valid:,filters:,assume-utreexo,ssl,uninstall,non-interactive,help -n "$0" -- "$@")
if [ $? -ne 0 ]; then
  show_usage
  exit 1
fi
...
interactive_ask_assume_valid() {
  if dialog --title "Floresta-Installer" --yesno "Do you want to use 'assume-valid'?\n\nThis option assumes that scripts before this height are validb." 15 60; then
