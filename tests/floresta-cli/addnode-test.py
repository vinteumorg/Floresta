...
class AddnodeTestWrapper:
    ...
    @staticmethod
    def test_should_utreexod_disconnect_and_floresta_await_for_it(test, nodes):
        """
        The test follows:
        ...
        """
        test.log(
            "=========== Testing should utreexod disconnect and floresta await for it..."
        )
        ...
        peer_info = nodes[0].rpc.get_peerinfo()
        test.assertEqual(len(peer_info), 1)
        test.assertEqual(peer_info[0]["services"], "ServiceFlags(NONE)")
        test.assertEqual(peer_info[0]["state"], "Awaiting")
        test.assertEqual(peer_info[0]["user_agent"], "")

    @staticmethod
    def test_should_utreexod_reconnect_and_floresta_await_for_it_be_ready(test, nodes):
        """
        The test follows:
    ...
    def run_test(test, nodes):
        ...
        AddnodeTestWrapper.test_should_utreexod_disconnect_and_floresta_await_for_it(
            test, nodes
        )
        AddnodeTestWrapper.test_should_utreexod_reconnect_and_floresta_await_for_it_be_ready(
            test, nodes
        )
