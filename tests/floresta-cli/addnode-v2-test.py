...
class AddnodeTestV2(FlorestaTestFramework):
    ...
    def set_test_params(self):
        ...
        AddnodeTestV2.nodes[0] = self.add_node(
            variant="florestad",
            extra_args=[f"--data-dir={self.data_dirs[0]}"],
        )

        AddnodeTestV2.nodes[1] = self.add_node(
            variant="bitcoind",
    ...
    def start_both_nodes(self):
        ...
        for i in self.nodes:
            ...
            self.run_node(AddnodeTestV2.nodes[i])

        return [self.get_node(i) for i in AddnodeTestV2.nodes]
    ...
    def test_should_floresta_add_bitcoind(self):
        ...
        floresta = self.get_node(AddnodeTestV2.nodes[0])
        bitcoind = self.get_node(AddnodeTestV2.nodes[1])
    ...
    def test_should_bitcoind_see_floresta(self):
        ...
        bitcoin_rpc = self.get_node(AddnodeTestV2.nodes[1]).rpc
        peer_info = bitcoin_rpc.get_peerinfo()
        self.assertEqual(len(peer_info), 1)
    ...
    def test_should_bitcoind_disconnect(self):
        ...
        # with the test
        floresta_rpc = self.get_node(AddnodeTestV2.nodes[0]).rpc
        self.stop_node(1)
        floresta_rpc.ping()
    ...
    def test_should_florestad_reconnect(self):
        ...
        self.run_node(AddnodeTestV2.nodes[1], timeout=300)

        # wait for the bitcoind node to be ready
        floresta_rpc = self.get_node(AddnodeTestV2.nodes[0]).rpc
        bitcoin_rpc = self.get_node(AddnodeTestV2.nodes[1]).rpc
        bitcoin_rpc.wait_for_connections(opened=True, timeout=PING_TIMEOUT)
        time.sleep(PING_TIMEOUT)
