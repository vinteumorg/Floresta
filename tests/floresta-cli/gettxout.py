    ...
    def run_test(self):
        ...
        for height in range(1, blocks):
            ...
            for tx in block["tx"]:
                ...
                txout_floresta = self.florestad.rpc.get_txout(tx, vout=0)
                txout_bitcoind = self.bitcoind.rpc.get_txout(tx, vout=0)

                for key in ("bestblock", "coinbase", "value"):
                    self.assertEqual(txout_floresta[key], txout_bitcoind[key])

                for key in ("address", "desc", "hex", "type"):
                    self.assertEqual(
                        txout_floresta["scriptPubKey"][key],
                        txout_bitcoind["scriptPubKey"][key],
                    )
                # rust-bitcoin decode the scriptPubKey as with OP_<Code> instead
                # hexadecimals separated by spaces. For example, to address
                # bcrt1q4gfcga7jfjmm02zpvrh4ttc5k7lmnq2re52z2y
                # the scriptPubKey for floresta is `OP_0 OP_PUSHBYTES_20 aa138477d24cb7b7a84160ef55af14b7bfb98143`
                # and for bitcoind is `0 aa138477d24cb7b7a84160ef55af14b7bfb98143`
                if txout_floresta["scriptPubKey"]["type"] == "witness_v0_keyhash":
                    asm_floresta = txout_floresta["scriptPubKey"]["asm"]
                    asm_bitcoind = txout_bitcoind["scriptPubKey"]["asm"]
                    asm_floresta = asm_floresta.split(" ")[-1:][0]
                    asm_bitcoind = asm_bitcoind.split(" ")[-1:][0]
                    self.assertEqual(asm_floresta, asm_bitcoind)
