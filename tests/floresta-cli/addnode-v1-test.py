...
class AddnodeTestV1(FlorestaTestFramework):
    ...
    def set_test_params(self):
        ...
        AddnodeTestV1.nodes[0] = self.add_node(
            variant="florestad",
            extra_args=[f"--data-dir={self.data_dirs[0]}"],
        )

        AddnodeTestV1.nodes[1] = self.add_node(
            variant="bitcoind",
            extra_args=[
    ...
    def start_both_nodes(self):
        ...
        """
        for i in AddnodeTestV1.nodes:
            self.log(f"=========== Starting node {i}...")
            self.run_node(AddnodeTestV1.nodes[i])

        return [self.get_node(i) for i in self.nodes]
    ...
    def test_should_floresta_add_bitcoind(self):
        ...
        floresta = self.get_node(AddnodeTestV1.nodes[0])
        bitcoind = self.get_node(AddnodeTestV1.nodes[1])
        bitcoind_port = bitcoind.get_port("p2p")

        # call the rpc
        result = floresta.rpc.addnode(
        ...
        peer_info = floresta.rpc.get_peerinfo()
    ...
    def test_should_bitcoind_see_floresta(self):
        ...
        # call the rpc
        bitcoind_host = bitcoind.get_host()
        bitcoind_port = bitcoind.get_port("p2p")
    ...
    def test_should_bitcoind_disconnect(self):
        ...
        # with the test
        floresta_rpc = self.get_node(AddnodeTestV1.nodes[0]).rpc
        self.stop_node(1)
        floresta_rpc.ping()
        ...
        peer_info = floresta_rpc.get_peerinfo()
    ...
    def test_should_florestad_reconnect(self):
        ...
        self.run_node(AddnodeTestV1.nodes[1])

        # wait for the bitcoind node to be ready
        floresta_rpc = self.get_node(AddnodeTestV1.nodes[0]).rpc
        bitcoind_rpc = self.get_node(AddnodeTestV1.nodes[1]).rpc
        bitcoind_rpc.wait_for_connections(opened=True, timeout=PING_TIMEOUT)
        time.sleep(PING_TIMEOUT)
    ...
    def test_should_floresta_remove_bitcoind(self):
        ...
        self.stop_node(1)
        bitcoind.rpc.wait_for_connections(opened=False)
    ...
    def test_should_bitcoind_not_see_floresta(self):
        ...
        bitcoind_rpc = self.get_node(AddnodeTestV1.nodes[1]).rpc
        peer_info = bitcoind_rpc.get_peerinfo()
        self.assertEqual(len(peer_info), 0)
