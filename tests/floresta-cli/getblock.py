...
class GetBlockTest(FlorestaTestFramework):
    ...
    def run_test(self):
        ...
        n_blocks = 21
        self.bitcoind.rpc.generate_block(n_blocks)

        timeout_secs = 10
        start = time.time()
        while self.florestad.rpc.get_block_count() < n_blocks:
            if time.time() - start > timeout_secs:
                self.stop()
                raise RuntimeError(
                    f"Timeout waiting for florestad to reach {n_blocks} blocks (>{timeout_secs}s)"
                )
            time.sleep(0.5)
