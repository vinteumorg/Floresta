...
class AddnodeTestV1Transport(FlorestaTestFramework):
    """
    ...
    `addnode ip[:port]` through a v1 transport protocol.
    """
    ...
    data_dirs = [
        os.path.normpath(
            os.path.join(
                ...
                "data",
                "floresta-cli-addnode-v1-transport",
                "node-0",
            )
        ),
        os.path.normpath(
    ...
    def set_test_params(self):
        ...
        """
        AddnodeTestV1Transport.nodes[0] = self.add_node(
            extra_args=[
                f"--data-dir={AddnodeTestV1Transport.data_dirs[0]}",
                f"--electrum-address={AddnodeTestV1Transport.electrum_addrs[0]}",
                f"--rpc-address={AddnodeTestV1Transport.rpc_addrs[0]}",
            ],
            rpcserver=REGTEST_RPC_SERVER,
            ssl=False,
        )

        AddnodeTestV1Transport.nodes[1] = self.add_node(
            extra_args=[
                f"--data-dir={AddnodeTestV1Transport.data_dirs[1]}",
                f"--electrum-address={AddnodeTestV1Transport.electrum_addrs[1]}",
                f"--rpc-address={AddnodeTestV1Transport.rpc_addrs[1]}",
            ],
            rpcserver={
                "host": "127.0.0.1",
    ...
    def start_both_nodes(self):
        ...
        """
        for i in AddnodeTestV1Transport.nodes:
            self.run_node(AddnodeTestV1Transport.nodes[i])

        return [self.get_node(i) for i in AddnodeTestV1Transport.nodes]
    ...
    def test_should_add_each_other_to_peers_list(self, nodes):
        ...
        - the transport protocol of "V1"
        - the kind of "regular";
        - the services of "ServiceFlags(NONE)";
        - oneshot of false.
        """
        peer_addresses = [AddnodeTestV1Transport.rpc_addrs[1 - i] for i in range(2)]
        for i in range(2):
    ...
    def test_should_not_add_each_other_to_peers_list(self, nodes):
        ...
        for i in range(2):
            result = nodes[i].rpc.addnode(
                node=peer_addresses[i], command="add", v2transport=False
            )
    ...
    def test_should_onetry_connection(self, nodes):
        ...
        for i in range(2):
            result = nodes[i].rpc.addnode(
                node=peer_addresses[i], command="onetry", v2transport=False
            )
    ...
    def run_test(self):
        ...
        1. Test `addnode` command with v1 transport in both nodes, and add them
        each other, then remove each other, then onetry each other;
        2. Check if the previous onetry command didnt added each other to
        the list;
        3. Test `addnode` command with v2 transport in both nodes, and add them
        each other, then remove each other, then onetry each other;
        4. Check if the previous onetry command didnt added each other to
        the list;
        """
